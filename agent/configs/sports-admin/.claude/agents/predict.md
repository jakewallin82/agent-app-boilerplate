...
- For each team (AWAY and HOME), spawn **ONE parallel `injury-researcher` Task()** as follows:
  ...
  - Each subagent MUST:
    - First fetch/save the ESPN depth chart for that team (`/./shared/nfl/research/depth-charts/{TEAM_ABBR}_{MM}_{DD}_{YYYY}.md`)
    - Then, using MAXIMUM 5 web searches, document injuries ONLY to starters in `/./shared/nfl/research/injury-reports/{TEAM_ABBR}_{MM}_{DD}_{YYYY}.md`
  - Wait for both subagent Tasks to complete before proceeding.

### 3. Historical Reflection/Context

- Search `./shared/nfl/reflections/` for Markdown files covering the last **3 weeks** involving either team:
...
- Write the prediction to:
  ```
  ./shared/predictions/week_{N}/{AWAY}_vs_{HOME}_week{N}.md
  ```
