#!/usr/bin/env python3
"""
NCAAM Odds Fetcher - Retrieves game and player prop odds from The Odds API

Usage:
    # Game odds
    python ncaam_odds_api.py games

    # All player props for all games
    python ncaam_odds_api.py props

    # Player props for a specific game (by team name)
    python ncaam_odds_api.py props --game "Duke"

    # Specific player prop
    python ncaam_odds_api.py prop --market player_points --player "Cooper Flagg"

    # Specific player prop with bookmaker
    python ncaam_odds_api.py prop --market player_points --player "Cooper Flagg" --book fanduel

Available Markets:
    Game: h2h, spreads, totals
    Scoring: player_points
    Combinations: player_points_rebounds_assists, player_points_rebounds, player_points_assists
    Individual Stats: player_rebounds, player_assists, player_blocks, player_steals
    Shooting: player_threes
...
# NCAAM Market definitions
NCAAM_GAME_MARKETS = ["h2h", "spreads", "totals"]

NCAAM_PLAYER_MARKETS = {
    "scoring": ["player_points"],
...
ALL_PLAYER_MARKETS = [m for markets in NCAAM_PLAYER_MARKETS.values() for m in markets]
        ...
        if event_id:
            response = await client.get(
                ...
                params={
                    "apiKey": api_key,
                    "regions": "us",
                    "markets": ",".join(NCAAM_GAME_MARKETS),
                    "oddsFormat": "american"
                }
            )
        else:
...
async def cmd_props(api_key: str, team: str | None = None, category: str | None = None):
    ...
    markets = ALL_PLAYER_MARKETS
    if category and category in NCAAM_PLAYER_MARKETS:
        markets = NCAAM_PLAYER_MARKETS[category]
...
async def main():
    parser = argparse.ArgumentParser(description="NCAAM Odds Fetcher")
    ...
    props_parser.add_argument("--category", choices=list(NCAAM_PLAYER_MARKETS.keys()), help="Filter by category")
