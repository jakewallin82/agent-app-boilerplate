#!/usr/bin/env python3
"""
NFL Odds Fetcher - Retrieves game and player prop odds from The Odds API

Usage:
    # Game odds
    python nfl_odds_api.py games

    # All player props for all games
    python nfl_odds_api.py props

    # Player props for a specific game (by team name)
    python nfl_odds_api.py props --game "Bills"

    # Specific player prop
    python nfl_odds_api.py prop --market player_rush_yds --player "Josh Allen"

    # Specific player prop with bookmaker
    python nfl_odds_api.py prop --market player_rush_yds --player "Josh Allen" --book fanduel
...
Available Markets:
    ...
    Defense: player_sacks, player_solo_tackles, player_defensive_interceptions
    TD Props: player_anytime_td, player_1st_td, player_last_td
...
# NFL Market definitions
NFL_GAME_MARKETS = ["h2h", "spreads", "totals"]
...
NFL_PLAYER_MARKETS = {
    ...
    "receiving": ["player_receptions", "player_reception_yds", "player_reception_tds"],
    "defense": ["player_sacks", "player_solo_tackles", "player_defensive_interceptions"],
    "touchdown": ["player_anytime_td", "player_1st_td", "player_last_td"],
}

ALL_PLAYER_MARKETS = [m for markets in NFL_PLAYER_MARKETS.values() for m in markets]
...
async def cmd_props(api_key: str, team: str | None = None, category: str | None = None):
    ...
    markets = ALL_PLAYER_MARKETS
    if category and category in NFL_PLAYER_MARKETS:
        markets = NFL_PLAYER_MARKETS[category]
...
async def main():
    parser = argparse.ArgumentParser(description="NFL Odds Fetcher")
    ...
    props_parser.add_argument("--category", choices=list(NFL_PLAYER_MARKETS.keys()), help="Filter by category")

    # prop command (single player lookup)
    prop_parser = subparsers.add_parser("prop", help="Get specific player prop")
    prop_parser.add_argument("--market", required=True, help="Market key (e.g., player_rush_yds)")
