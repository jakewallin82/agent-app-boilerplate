#!/usr/bin/env python3
"""
NHL Odds Fetcher - Retrieves game and player prop odds from The Odds API

Usage:
    # Game odds
    python nhl_odds_api.py games

    # All player props for all games
    python nhl_odds_api.py props

    # Player props for a specific game (by team name)
    python nhl_odds_api.py props --game "Maple Leafs"

    # Specific player prop
    python nhl_odds_api.py prop --market player_points --player "Auston Matthews"

    # Specific player prop with bookmaker
    python nhl_odds_api.py prop --market player_points --player "Auston Matthews" --book fanduel

Available Markets:
    Game: h2h, spreads, totals
    Scoring: player_points, player_goals, player_assists
    Performance: player_shots_on_goal, player_blocked_shots, player_power_play_points
    Goalie: player_total_saves
    Goal Scorer: player_goal_scorer_first, player_goal_scorer_anytime, player_goal_scorer_last
...
# NHL Market definitions
NHL_GAME_MARKETS = ["h2h", "spreads", "totals"]

NHL_PLAYER_MARKETS = {
    "scoring": ["player_points", "player_goals", "player_assists"],
    "performance": ["player_shots_on_goal", "player_blocked_shots", "player_power_play_points"],
    "goalie": ["player_total_saves"],
    "goal_scorer": ["player_goal_scorer_first", "player_goal_scorer_anytime", "player_goal_scorer_last"],
}

ALL_PLAYER_MARKETS = [m for markets in NHL_PLAYER_MARKETS.values() for m in markets]
...
def format_player_props_minimal(data: dict[str, Any], player_filter: str | None = None) -> str:
    ...
    for market in bk.get("markets", []):
        ...
        for player, odds_list in players.items():
            if len(odds_list) == 1:
                # Yes/No market (like goal scorer)
                o = odds_list[0]
...
async def cmd_props(api_key: str, team: str | None = None, category: str | None = None):
    ...
    markets = ALL_PLAYER_MARKETS
    if category and category in NHL_PLAYER_MARKETS:
        markets = NHL_PLAYER_MARKETS[category]
...
async def main():
    parser = argparse.ArgumentParser(description="NHL Odds Fetcher")
    ...
    props_parser.add_argument("--category", choices=list(NHL_PLAYER_MARKETS.keys()), help="Filter by category")
