...
- For each team (AWAY and HOME), spawn **ONE parallel `rest-predictor-nba` Task()** as follows:
  ...
  - Each subagent MUST:
    ...
    - Output to `/data/nba/research/rest-tracker/{TEAM_ABBR}_{YYYY-MM-DD}.md`
...
- For each team, check for injury reports:
  ...
  - Document key injuries to `/data/nba/research/injury-reports/{TEAM_ABBR}_{YYYY-MM-DD}.md`
  - Focus on starters and key rotation players

### 4. Historical Reflection/Context

- Search `data/nba/reflections/` for Markdown files covering the last **7 days** involving either team:
