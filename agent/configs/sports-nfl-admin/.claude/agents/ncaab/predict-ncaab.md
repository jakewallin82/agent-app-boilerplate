...
- Spawn **ONE `kenpom-analyzer-ncaab` Task()** as follows:
  ...
  - The subagent MUST:
    ...
    - Output to `/data/ncaab/research/kenpom-analysis/{AWAY}_vs_{HOME}_{YYYY-MM-DD}.md`
...
- NCAAB has limited reliable injury reporting, but check if available:
  ...
  - Document any confirmed injuries to `/data/ncaab/research/injury-reports/{TEAM_ABBR}_{YYYY-MM-DD}.md`
  - Focus on key starters if information is available

### 4. Historical Reflection/Context

- Search `data/ncaab/reflections/` for Markdown files covering the last **7 days** involving either team:
